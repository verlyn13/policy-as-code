...
deny[msg] {
    ...
    not required in input.packages[_]
...
deny[msg] {
    ...
    not required in input.services.enabled[_]
...
deny[msg] {
    input.network.interfaces[_].ip
    not regex.match("^([0-9]{1,3}\\.){3}[0-9]{1,3}$", input.network.interfaces[_].ip)
    msg := sprintf("Invalid IP address format: %s", 
        [input.network.interfaces[_].ip])
}
